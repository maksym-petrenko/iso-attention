...
#SBATCH --mem=64GB
#SBATCH --time=4:00:00
#SBATCH --output=logs/custom_job_%j.out
#SBATCH --error=logs/custom_job_%j.err
...
echo "Starting CUSTOM model training on $(hostname) with MASTER_ADDR=$MASTER_ADDR:$MASTER_PORT"
echo "Using network interface: $GLOO_SOCKET_IFNAME"

# Run distributed training with CUSTOM model
...
torchrun \
    --standalone \
    --nproc_per_node=1 \
    -m scripts.custom_train --depth=12 --device_batch_size=4
